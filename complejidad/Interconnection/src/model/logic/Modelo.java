...
public class Modelo {
	...
	public String obtenerLlave(ILista lista1) {
		String llave = "";
		try {
			int max = 0;
		...
		} catch (PosException | VacioException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		return llave;
	...
	public Object[] req4Candidatos(ILista lista2, int distancia, ILista candidatos, ITablaSimbolos tabla) {
		try {
		...
		return new Object[] { candidatos, tabla };
	}

	public void req4Fragmento(String fragmento, ILista unificado, ITablaSimbolos tabla, int distancia) {
		...
		} catch (VacioException | PosException e) {
			e.printStackTrace();
		}
	...
	public String req4String() {
		...
		int distancia = 0;
		String llave = obtenerLlave(lista1);
		...
		Object response = req4Candidatos(lista2, distancia, candidatos, tabla);
		candidatos = (ILista) ((Object[]) response)[0];
		tabla = (ITablaSimbolos) ((Object[]) response)[1];
		ILista unificado = unificar(candidatos, "Vertice");
		req4Fragmento(fragmento, unificado, tabla, distancia);
	...
	public ILista unificarVertice(ILista lista) {
	...
	public ILista unificarNoVertice(ILista lista) {
		ILista lista2 = new ArregloDinamico(1);
		Comparator<Country> comparador = null;
	...
	public ILista unificar(ILista lista, String criterio) {
		...
		if (criterio.equals("Vertice")) {
			lista2 = unificarVertice(lista);
		} else {
			lista2 = unificarNoVertice(lista);
		}
		return lista2;
	}
	...
	public Iterable<CSVRecord> leerArchivo(String ruta) throws IOException {
		Reader in = new FileReader(ruta);
		Iterable<CSVRecord> records = CSVFormat.RFC4180.withHeader().parse(in);
		return records;
	}

	public void procesarCountries(Iterable<CSVRecord> records) {
	...
	public void procesarLandings(Iterable<CSVRecord> records) {
		for (CSVRecord record : records) {
	...
	public Country validarPais(String nombrepais, Landing landing1, String cableid) {
		Country pais = null;
		if (nombrepais.equals("Côte d'Ivoire")) {
			pais = (Country) paises.get("Cote d'Ivoire");
		} else {
			pais = (Country) paises.get(nombrepais);
		}

		if (pais != null) {
			float weight = distancia(pais.getLongitude(), pais.getLatitude(), landing1.getLongitude(),
					landing1.getLatitude());

			grafo.addEdge(pais.getCapitalName(), landing1.getLandingId() + cableid, weight);
		}
		return pais;
	}

	public void validarLandings(Landing landing1, Landing landing2, String cableid) {
		if (landing1 != null) {
			if (landing2 != null) {
	...
	public void crearListaTop(Landing landing1, Landing landing2, Vertex vertice1, Vertex vertice2) {
		try {
	...
	public void procesarConections(Iterable<CSVRecord> records) {
		for (CSVRecord record3 : records) {
			...
			Country pais1 = validarPais(nombrepais1, landing1, cableid);
			Country pais2 = validarPais(nombrepais2, landing2, cableid);

			validarLandings(landing1, landing2, cableid);

			crearListaTop(landing1, landing2, vertice1, vertice2);
		}
	}

	public void procesarLandingTabla(ILista valores) {
		try {
	...
	public void cargar() throws IOException {
		...
		Iterable<CSVRecord> records = leerArchivo("./data/countries.csv");
		procesarCountries(records);

		Iterable<CSVRecord> records2 = leerArchivo("./data/landing_points.csv");
		procesarLandings(records2);

		Iterable<CSVRecord> records3 = leerArchivo("./data/connections.csv");
		procesarConections(records3);

		ILista valores = landingidtabla.valueSet();
		procesarLandingTabla(valores);
